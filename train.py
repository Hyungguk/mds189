...
train_dataset = Mds189(label_file_train,loader=default_loader,transform=transforms.Compose([
                                               transforms.Pad(5),    # TODO: if you want to pad your images
                                               transforms.Resize(100), # TODO: if you want to resize your images
                                               transforms.ToTensor(),
...
class NeuralNet(nn.Module):
    def __init__(self):
        ...
        #self.conv1 = nn.Conv2d(1, 6, 5) # you should review the definition of nn.Conv2d online
    ...
    def forward(self, x):
        ...
        #print("shape after second max pooling:", x.shape)
        x = x.view(-1, self.num_flat_features(x))
        #print("shape after view:", x.shape)
        x = F.relu(self.fc1(x))
        x = self.drop1(x)
        #print("shape after first relu:", x.shape)
        x = F.relu(self.fc2(x))
        #print("shape after second relu:", x.shape)
        x = self.fc3(x)
        x = self.drop2(x)
        x = self.fc4(x)
        #print("shape after last:", x.shape)
        return x
